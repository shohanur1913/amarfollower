...
$id = (int)($_GET['id'] ?? 0);
$platform = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM platforms WHERE id = $id"));

if(!$platform) { echo "Platform not found!"; exit; }
?>
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-xl font-bold uppercase tracking-tight">Edit Platform #<?php echo $id; ?></h2>
    </div>
    ...
    <div class="bg-white dark:bg-[#18181b] border border-slate-200 dark:border-slate-800 rounded-md shadow-sm overflow-hidden">
        <form action="api/platform_actions.php" method="POST" class="p-8 space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="<?php echo $platform['name']; ?>" required class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md px-4 py-2.5 font-bold text-sm outline-none focus:border-primary">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Sort Order</label>
                    <input type="number" name="sort_order" value="<?php echo $platform['sort_order']; ?>" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md px-4 py-2.5 font-bold text-sm outline-none">
                </div>
                <div class="space-y-2">
                    ...
                    <select name="status" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md px-4 py-2.5 font-bold text-sm outline-none">
                        <option value="1" <?php echo $platform['status'] == 1 ? 'selected' : ''; ?>>Active</option>
                        <option value="0" <?php echo $platform['status'] == 0 ? 'selected' : ''; ?>>Disabled</option>
                    </select>
                </div>
            </div>
    ...
    <p class="text-[9px] text-slate-400 mt-1 italic leading-none">
        Brands use: <span class="font-bold">fa-brands fa-instagram</span> | Solid icons use: <span class="font-bold">fa-solid fa-user</span>
            ...
            <button type="submit" class="w-full bg-primary text-white font-black py-3 rounded-md uppercase tracking-widest text-[11px] shadow-md transition active:scale-95">Update Platform</button>
        </form>
    </div>
</div>